...
-- Create additional security roles if they don't exist
DO $$
BEGIN
    ...
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'system_maintenance') THEN
        CREATE ROLE system_maintenance;
    END IF;
END $$;

-- Grant permissions to analytics reader
GRANT SELECT ON table_cards TO analytics_reader;
GRANT SELECT ON bar_orders TO analytics_reader;
GRANT SELECT ON bar_order_items TO analytics_reader;
GRANT SELECT ON recharges TO analytics_reader;
GRANT SELECT ON app_transaction_log TO analytics_reader;
GRANT SELECT ON nfc_scan_log TO analytics_reader;
GRANT SELECT ON transaction_analytics TO analytics_reader;
GRANT SELECT ON monitoring_dashboard_summary TO analytics_reader;
GRANT SELECT ON system_health_trend TO analytics_reader;

-- Grant permissions to system maintenance
GRANT SELECT, DELETE ON app_transaction_log TO system_maintenance;
GRANT SELECT, DELETE ON nfc_scan_log TO system_maintenance;
GRANT SELECT, DELETE ON monitoring_events TO system_maintenance;
GRANT SELECT, DELETE ON idempotency_keys TO system_maintenance;
GRANT SELECT, DELETE ON nfc_operation_locks TO system_maintenance;
GRANT EXECUTE ON FUNCTION comprehensive_system_cleanup TO system_maintenance;
GRANT EXECUTE ON FUNCTION cleanup_expired_nfc_resources TO system_maintenance;
GRANT EXECUTE ON FUNCTION cleanup_monitoring_data TO system_maintenance;

-- =====================================================
-- 7. MISSING CONSTRAINTS AND VALIDATIONS
-- =====================================================
...
DECLARE
    ...
    required_tables TEXT[] := ARRAY[
        'table_cards', 'bar_orders', 'bar_order_items', 'recharges',
        'idempotency_keys', 'app_transaction_log', 'nfc_scan_log',
        'nfc_operation_locks', 'monitoring_events', 'system_health_snapshots',
        'alert_history'
    ];
    table_name TEXT;
BEGIN
...
BEGIN
    ...
    RAISE NOTICE '=================================================';
    RAISE NOTICE 'Migration: 20250615_124721_consolidated_missing_components.sql';
    ...
    RAISE NOTICE '- 2 Additional Security Roles';
